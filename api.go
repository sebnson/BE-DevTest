...
func startAPIServer() {
	...
	http.HandleFunc("/token/priceAndsource", handleTokenPriceAndSourceRequest)
...
func handleTokenPriceAndSourceRequest(w http.ResponseWriter, r *http.Request) {
	// 요청 파라미터에서 토큰 심볼을 읽기 (예: /token/price?symbol=USDT)
	tokenSymbol := r.URL.Query().Get("symbol")
	tokenSource := r.URL.Query().Get("source")

	// 데이터베이스에서 토큰 가격 정보 조회
	priceInfo, err := getTokenPriceAndSource(tokenSymbol, tokenSource)
	if err != nil {
		http.Error(w, "Error getting token price and source", http.StatusInternalServerError)
		return
	}
